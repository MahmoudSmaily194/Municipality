...
const Service = () => {
  ...
  const pagesNb = Math.ceil(services.length / 15);
  const FirstIndex= (currentPage -1 ) * 15;
  const currentItems=services.slice(FirstIndex,FirstIndex+15);
  return (
    <div className={style.serv_page_con}>
      <div className={style.serv_page}>
       ...
       <Pagination NbOfPages={pagesNb} setCurrentPage={setCurrentPage} currentPage={currentPage}/>
        </div>
      </div>
    </div>
  );
